    ...
    if os.path.exists(config_file):
        try:
            ...
            if te_rule:
                ...
                print(f"   - 필수 메타데이터: {', '.join(te_rule.get('required_metadata', []))}")
                print(f"   - 필수 테이블 필드: {', '.join(te_rule.get('required_table_fields', []))}")
                print(f"   - 이미지 필요: {'예' if te_rule.get('required_images') else '아니오'}")
